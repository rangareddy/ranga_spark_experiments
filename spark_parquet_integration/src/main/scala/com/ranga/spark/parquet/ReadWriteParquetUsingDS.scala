...
object ReadWriteParquetUsingDS extends App {
    ...
    val outputParquetFile = "output_parquet_data"
    ...
    def writeParquet(session: SparkSession) = {
        ...
        val inputDF = session.read.parquet(parquetFile).drop("original_year", "original_month", "original_day")
        ...
        yearDF.write.mode("overwrite").partitionBy("year", "month", "day", "hour").parquet(outputParquetFile)
    ...
    def readParquet(session: SparkSession) = {
        ...
        val parquestDF = session.read.parquet(outputParquetFile)
        ...
        val empDF2 = session.read.parquet(outputParquetFile+"/year=2014/month=5/day=14")
